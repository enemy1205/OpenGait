...
def evaluate_drone_dataset(data, dataset, metric="euc", cross_view_gallery=False):
    feature, label, seq_type, view = (
        data["embeddings"],
        data["labels"],
        data["types"],
...
def evaluate_image_rec(data, dataset, return_ssim_psnr=False, clip_range=[0, 1]):
    ...
    if return_ssim_psnr:
        ...
        msg_mgr.log_info(
            f"scalar/test_accuracy/mse:{mse}   scalar/test_accuracy/mae:{mae}"
        )
        msg_mgr.log_info(
            f"scalar/test_accuracy/ssim:{ssim}   scalar/test_accuracy/psnr:{psnr}"
        )
        return {
            "scalar/test_accuracy/mse": mse,
            "scalar/test_accuracy/mae": mae,
            "scalar/test_accuracy/ssim": ssim,
            "scalar/test_accuracy/psnr": psnr,
        }
    else:
        msg_mgr.log_info(
            f"scalar/test_accuracy/mse:{mse}   scalar/test_accuracy/mae:{mae}"
...
def evaluate_gait_rec(
    ...
    if return_ssim_psnr:
        ...
        result_dict.update(
            {"scalar/test_accuracy/mse": mse, "scalar/test_accuracy/mae": mae}
        )
    return result_dict
