...
class DroneDataSet(tordata.Dataset):
    def __init__(self, data_cfg, training):
        """
        seqs_info: the list with each element indicating
                        a certain gait sequence presented as [label, type, camera_height, clip, paths];
        """
        ...
        self.camera_height_set = sorted(list(set(self.views_list)))
    ...
    def __loader__(self, paths):
        ...
        for idx, data in enumerate(data_list):
            if len(data) != len(data_list[0]):
                raise ValueError(
                    "Each input data({}) should have the same length.".format(
                        paths[idx]
                    )
                )
            if len(data) == 0:
                raise ValueError(
                    "Each input data({}) should have at least one element.".format(
                        paths[idx]
                    )
                )
        return data_list
    ...
    def __getitem__(self, idx):
        # self.seqs_info [[subject,type,view,pkl_paths],[subject,type,view,pkl_paths],...] 其中每一组subject,type,view会有区别
        # self.seqs_info[idx] [subject,type,view,pkl_paths]
        # self.seqs_info[idx][-1] 则是获得pkl文件实际路径,一般只有一个，但Drone数据集有10个
        if not self.cache:
            data_list = self.__loader__([random.choice(self.seqs_info[idx][-1])])
    ...
    def __dataset_parser(self, data_config, training):
        ...
        miss_pids = [
            label for label in label_list if label not in (train_set + test_set)
        ]
        msg_mgr = get_msg_mgr()
        ...
        def get_seqs_info_list(label_set):
            ...
            for lab in label_set:
                for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                    for cam_h in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
                        seq_info = [lab, typ, cam_h]
                        ...
                        if seq_dirs != []:
                            ...
                            if data_in_use is not None:
                                seq_dirs = [
                                    dir
                                    for dir, use_bl in zip(seq_dirs, data_in_use)
                                    if use_bl
                                ]
                            seqs_info_list.append([*seq_info, seq_dirs])
                        else:
                            msg_mgr.log_debug(
                                "Find no .pkl file in %s-%s-%s-%s." % (lab, typ, cam_h)
                            )
            return seqs_info_list

        self.seqs_info = (
            get_seqs_info_list(train_set) if training else get_seqs_info_list(test_set)
        )
