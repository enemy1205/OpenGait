...
class LossAggregator(nn.Module):
    ...
    def __init__(self, loss_cfg) -> None:
        ...
        self.losses = nn.ModuleDict(
            {loss_cfg["log_prefix"]: self._build_loss_(loss_cfg)}
            if is_dict(loss_cfg)
    ...
    def _build_loss_(self, loss_cfg):
        ...
        valid_loss_arg = get_valid_args(Loss, loss_cfg, ["type", "gather_and_scale"])
        ...
        for k, v in training_feats.items():
            if k in self.losses:
                ...
                if isinstance(v, dict):
                    raise ValueError(
                        "The key %s in -Trainng-Feat- should be stated in your loss_cfg as log_prefix."
                        % k
                    )
                elif is_tensor(v):
                    _ = v.mean()
...
class LossAggregator_Mul(nn.Module):
    """The loss aggregator.
