...
class MSTE_3D(nn.Module):
    ...
    def get_short_term(self, x):
        temp = self.short_term[0](x)
        short_term_feature = temp + self.short_term[1](temp)
        return short_term_feature

    def get_long_term(self, x):
        n, c, s, h, w = x.size()
        x = x.permute(0, 3, 4, 1, 2).contiguous()
        pred_score = self.score(x.view(n, h * w * c, s)).view(n, h, w, c, s)
        long_term_feature = x.mul(pred_score).sum(-1).div(pred_score.sum(-1))
        long_term_feature = long_term_feature.unsqueeze(1).repeat(1, s, 1, 1, 1)
...
class SSFL_SP(nn.Module):
    ...
    def forward(self, t_f, t_s, t_l):
        ...
        return weighted_part_vector, selected_part_feature
