...
class ThresholdBinary(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        binary_output = torch.where(
            sigmoid_output >= self.threshold, torch.tensor(1), torch.tensor(0)
        )
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, model_cfg, init_weights=True):
        ...
        if model_cfg is None:
            ...
            channel = model_cfg["G_channel"]
            stack_num = model_cfg["G_stack_num"]
            patchsize = model_cfg["G_patchsize"]
        blocks = []
        ...
        # self.binary = BinaryActivation(0.6)
        if init_weights:
            self.init_weights()
...
class Discriminator(BaseNetwork):
    def __init__(
        ...
        self.use_sigmoid = (
            model_cfg["use_sigmoid"]
            if model_cfg["use_sigmoid"] is not None
            else use_sigmoid
        )
