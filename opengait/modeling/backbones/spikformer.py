...
class SpikformerBlock(nn.Module):
    def __init__(self,
                in_channels=2,embed_dim =64, num_heads=[1, 2, 4], 
                mlp_ratios=[4, 4, 4],drop_rate=0., drop_path_rate=0.,
                 norm_layer=nn.LayerNorm,depths=[6, 8, 6]):
        super().__init__()
        self.depths = depths
        ...
        self.block = nn.ModuleList([Block(
            dim=int(embed_dim), num_heads=num_heads[j], mlp_ratio=mlp_ratios[j],
             drop=drop_rate, drop_path=dpr[j], norm_layer=norm_layer)
