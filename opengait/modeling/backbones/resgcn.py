...
class ResGCNModule(nn.Module):
    ...
    def __init__(
        ...
        self.scn = spatial_block(
            in_channels, out_channels, max_graph_distance, block_res, reduction
        )
        if in_channels == out_channels and is_main:
            tcn_stride = True
        else:
            tcn_stride = False
        self.tcn = temporal_block(
            out_channels,
...
class ResGCN(nn.Module):
    ...
    def __init__(
        self, input_num, input_branch, main_stream, num_class, reduction, block, graph
    ):
        ...
        for i in range(len(main_stream) - 1):
            if main_stream[i] == main_stream[i + 1]:
                ...
                stride = 2
            if i == 0:
                main_stream_list.append(
                    ResGCNModule(
                        main_stream[i] * input_num,
                        main_stream[i + 1],
                        block,
                        graph,
                        stride=1,
