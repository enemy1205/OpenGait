...
class GaitGraph1(BaseModel):
    ...
    def forward(self, inputs):
        ...
        if self.training:
            x_input = torch.cat(
                [x_input[:, : int(T / 2), ...], x_input[:, int(T / 2) :, ...]], dim=0
            )  # [8, 60, 17, 1, 3]
        elif self.tta:
        ...
        x = F.normalize(
            x, dim=1, p=2
        )  # norm #only for GaitGraph1 # Remove from GaitGraph2
        ...
        retval = {
            ...
            "visual_summary": {
                "image/pose": pose.view(
                    N * T, 1, I * V, C
                ).contiguous()  # visualization
            },
            "inference_feat": {"embeddings": embed},  # for metric
        }
        return retval
