...
def padding_resize(x, ratios, target_h, target_w):
    ...
    pad_left = torch.where(
        need_padding_mask, (w - need_w) // 2, torch.tensor(0).to(x.device)
    )
    pad_right = torch.where(
        need_padding_mask, w - need_w - pad_left, torch.tensor(0).to(x.device)
    ).tolist()
...
class BigGait__Dinov2_Gaitbase(BaseModel):
    ...
    def init_Mask_Branch(self):
        ...
        self.msg_mgr.log_info(
            "SegmentationBranch Count: {:.5f}M".format(n_parameters / 1e6)
        )
    ...
    def init_parameters(self):
        ...
        self.msg_mgr.log_info(
            "Expect backbone Count: {:.5f}M".format(n_parameters / 1e6)
        )
    ...
    def preprocess(self, sils, image_size, mode="bilinear"):
        ...
        return F.interpolate(
            sils, (image_size * 2, image_size), mode=mode, align_corners=False
        )
    ...
    def get_body(self, mask):
        ...
        def judge_edge(image, edge=1):
            # [nxs,h,w]
            edge_pixel_count = image[:, :edge, :].sum(dim=(1, 2)) + image[
                :, -edge:, :
            ].sum(dim=(1, 2))
            return edge_pixel_count > (image.size(2)) * edge

        condition_mask = (
            torch.round(mask[..., 0]) - mask[..., 0].detach() + mask[..., 0]
        )
    ...
    def connect_loss(self, images, n, s, c):
        ...
        gradient_x = F.conv2d(
            images,
            torch.Tensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]])[None, None, ...]
            .repeat(1, c, 1, 1)
            .to(images.dtype)
            .to(images.device),
            padding=1,
        ...
        gradient_y = F.conv2d(
            images,
            torch.Tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]])[None, None, ...]
            .repeat(1, c, 1, 1)
            .to(images.dtype)
            .to(images.device),
            padding=1,
        ...
        loss_connectivity = (
            torch.sum(torch.abs(gradient_x)) + torch.sum(torch.abs(gradient_y))
    ...
    def get_edge(self, sils, threshold=1):
        ...
        dilated_mask = morph.dilation(
            mask_sils, kernel.to(sils.device)
        ).detach()  # Dilation
        kernel = torch.ones((5, 5))
        eroded_mask = morph.erosion(
            dilated_mask, kernel.to(sils.device)
        ).detach()  # Erosion
        edge_mask = (dilated_mask > 0.5) ^ (eroded_mask > 0.5)
        sils = edge_mask * sils + (eroded_mask > 0.5) * torch.ones_like(
            sils, dtype=sils.dtype, device=sils.device
        )
        return sils

    def diversity_loss(self, images, max_p):
        # [ns, hw, c]
        p = torch.sum(images, dim=1) / (torch.sum(images, dim=(1, 2)) + 1e-6).view(
            -1, 1
        ).repeat(1, max_p)
        entropies = -torch.sum(p * torch.log2(p + 1e-6), dim=1)
        max_p = (
            torch.Tensor([1 / max_p]).repeat(max_p).to(images.dtype).to(images.device)
        )
    ...
    def forward(self, inputs):
        ...
        with torch.no_grad():
            ...
            if h == 2 * w:
                outs = self.preprocess(
                    sils, self.image_size
                )  # [ns,c,448,224]    if have used pad_resize for input images
            else:
                outs = self.preprocess(
                    padding_resize(sils, ratios, 256, 128), self.image_size
                )  # [ns,c,448,224]    if have not used pad_resize for input images
            ...
            outs_last1 = rearrange(
                outs_last1.view(n, s, self.image_size // 7, self.image_size // 14, -1),
                "n s h w c -> (n s) c h w",
            ).contiguous()
            outs_last4 = rearrange(
                outs_last4.view(n, s, self.image_size // 7, self.image_size // 14, -1),
                "n s h w c -> (n s) c h w",
            ).contiguous()
            ...
            outs_last1 = rearrange(
                outs_last1.view(n, s, -1, self.sils_size * 2, self.sils_size),
                "n s c h w -> (n s) (h w) c",
            ).contiguous()
            outs_last4 = rearrange(
                outs_last4.view(n, s, -1, self.sils_size * 2, self.sils_size),
                "n s c h w -> (n s) (h w) c",
            ).contiguous()
        ...
        foreground = (
            torch.zeros_like(mask, dtype=fore_feat.dtype, device=fore_feat.device)
            .view(-1, 1)
            .repeat(1, self.mask_dim)
        )
        ...
        foreground = self.get_body(
            foreground.view(n * s, self.sils_size * 2, self.sils_size, self.mask_dim)
        ).view(
            n * s, -1
        )  # [n*s,h*w]
        ...
        denosing = (
            torch.zeros_like(foreground, dtype=den_feat.dtype, device=den_feat.device)
            .view(-1, 1)
            .repeat(1, self.denoising_dim)
        )
        denosing[foreground.view(-1) != 0] = den_feat
        loss_connectivity_part = self.connect_loss(
            denosing.view(n * s, -1, self.denoising_dim)[..., :-1].permute(0, 2, 1),
            n,
            s,
            (self.denoising_dim - 1),
        )
        loss_diversity_part = self.diversity_loss(
            denosing.view(n * s, -1, self.denoising_dim), self.denoising_dim
        )
        del den_feat
        ...
        appearance = (
            torch.zeros_like(foreground, dtype=app_feat.dtype, device=app_feat.device)
            .view(-1, 1)
            .repeat(1, self.app_dim)
        )
        ...
        if self.training:
            try:
                vis_num = min(5, n * s)
                vis_mask = (
                    foreground.view(n * s, self.sils_size * 2 * self.sils_size, -1)[
                        :vis_num
                    ]
                    .detach()
                    .cpu()
                ...
                vis_denosing = pca_image(
                    data={
                        "embeddings": denosing.view(
                            n * s, self.sils_size * 2 * self.sils_size, -1
                        )[:vis_num]
                        .detach()
                        .cpu()
                        .numpy()
                ...
                vis_appearance = pca_image(
                    data={
                        "embeddings": appearance.view(
                            n * s, self.sils_size * 2 * self.sils_size, -1
                        )[:vis_num]
                        .detach()
                        .cpu()
            ...
            for i in mask_idx:
                ...
                for j in idx:
                    feat_list[j][i] = torch.zeros_like(
                        feat_list[j][i],
                        device=feat_list[j].device,
                        dtype=feat_list[j].dtype,
                    )

        # get embeding
        embed_1, logits = self.gait_net(
            denosing.view(n, s, self.sils_size * 2, self.sils_size, self.denoising_dim)
            .permute(0, 4, 1, 2, 3)
            .contiguous(),
            appearance.view(n, s, self.sils_size * 2, self.sils_size, self.app_dim)
            .permute(0, 4, 1, 2, 3)
            .contiguous(),
            seqL,
        )
            ...
            retval = {
                ...
                "visual_summary": {
                    ...
                    "image/denosing": self.min_max_norm(
                        rearrange(
                            torch.from_numpy(vis_denosing).float(),
                            "n s c h w -> (n s) c h w",
                        ).contiguous()
                    ),
                    "image/appearance": self.min_max_norm(
                        rearrange(
                            torch.from_numpy(vis_appearance).float(),
                            "n s c h w -> (n s) c h w",
                        ).contiguous()
                    ),
                },
