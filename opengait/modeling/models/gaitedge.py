...
class Segmentation(BaseModel):
    ...
    def forward(self, inputs):
        ...
        retval = {
            "training_feat": {"bce": {"logits": logits, "labels": sils}},
            "visual_summary": {
                "image/sils": sils,
                "image/logits": logits,
                "image/pred": pred,
            },
            "inference_feat": {"pred": pred, "mask": sils},
        }
        return retval
...
class GaitEdge(GaitGL):
    ...
    def build_network(self, model_cfg):
        ...
        self.kernel = torch.ones((model_cfg["kernel_size"], model_cfg["kernel_size"]))
    ...
    def finetune_parameters(self):
        ...
        return [
            {"params": fine_tune_params, "lr": self.seg_lr},
    ...
    def preprocess(self, sils):
        dilated_mask = (
            morph.dilation(sils, self.kernel.to(sils.device)).detach()
        ) > 0.5  # Dilation
        eroded_mask = (
            morph.erosion(sils, self.kernel.to(sils.device)).detach()
        ) > 0.5  # Erosion
    ...
    def forward(self, inputs):
        ...
        if self.is_edge:
            ...
            if self.align:
                cropped_logits = self.gait_align(logits, mask, ratios)
            else:
        ...
        retval["visual_summary"]["image/roi"] = cropped_logits.view(n * s, 1, H, W)
