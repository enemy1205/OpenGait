...
from ..backbones.fuse_former import deconv,TransformerBlock,SoftComp,SoftSplit,AddPosEmb,Encoder,Discriminator
...
class FFormerGait_3Loss(BaseModel):
    ...
    def get_optimizer(self, optimizer_cfg):
        ...
        for i in range(5): 
            if hasattr(self, 'layer%d'%i): 
                params_list.append(
                    {'params': getattr(self, 'layer%d'%i).parameters(), 'lr': optimizer_cfg['lr'], 'weight_decay': optimizer_cfg['weight_decay']}
                )
