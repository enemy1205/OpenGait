...
class FFormerRecGait(BaseModel):
    ...
    def forward(self, inputs):
        ...
        enc_feat = out3.transpose(1,2).reshape(b*t,-1,h//4,w//4)
...
class FFormerRecGait_enc(BaseModel):
    def build_network(self, model_cfg):
    ...
    def forward(self, inputs):
        ...
        gen_vid_feat = self.netDis(rec_sil)
        out4 = self.layer4(out3) # [b,512,t,h/4,w/4]
        out5 = self.fusion(enc_feat.view(b,t,-1,h//4,w//4).transpose(1,2),out3)
        # Temporal Pooling, TP
        outs_1 = self.TP(out4, seqL, options={"dim": 2})[0]  # [n, c, h, w]
        outs_2 = self.TP(out5, seqL, options={"dim": 2})[0]  # [n, c, h, w]
        # Horizontal Pooling Matching, HPM
        feat_1 = self.HPP(outs_1)
