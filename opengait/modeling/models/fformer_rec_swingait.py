        ...
        if self.downsample == False:
            ...
            if self.downsample is not None:
                ...
                self.downsample = nn.Sequential(
                    norm_layer(dim), nn.Linear(dim, 2 * dim, bias=False)
                )
...
class FFormerSwinGait(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.layer2 = self.make_layer(
            BasicBlockP3D, channels[1], stride=[2, 2], blocks_num=layers[1], mode="p3d"
        )
        ...
        self.FCs = SeparateFCs(
            model_cfg["SeparateBNNecks"]["parts_num"], in_channels=256, out_channels=128
        )
        ...
        params_list = [
            {
                "params": [
                    p
                    for n, p in transformer_params
                    if any(nd in n for nd in transformer_no_decay)
                ],
                "lr": optimizer_cfg["lr"],
                "weight_decay": 0.0,
            },
            {
                "params": [
                    p
                    for n, p in transformer_params
                    if not any(nd in n for nd in transformer_no_decay)
                ],
                "lr": optimizer_cfg["lr"],
    ...
    def forward(self, inputs):
        if self.training:
            adjust_learning_rate(
                self.optimizer, self.iteration, T_max_iter=self.T_max_iter
            )
...
def adjust_learning_rate(
    optimizer, iteration, iteration_per_epoch=1000, T_max_iter=10000, min_lr=1e-6
):
    """Decay the learning rate based on schedule"""
    if iteration < T_max_iter:
        if iteration % iteration_per_epoch == 0:
            alpha = 0.5 * (1.0 + math.cos(math.pi * iteration / T_max_iter))
