...
class DeepGaitV2(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.rec_model = SimVP(model_cfg['SimVP'])
        rec_model_param = torch.load(model_cfg['SimVP_PreTrain'])['model']
        self.rec_model.load_state_dict(rec_model_param)
        for param in self.rec_model.named_parameters():
            param[1].requires_grad = False
        print('model param load successfully !')
    ...
    def forward(self, inputs):
        ...
        # sils = ipts[1].unsqueeze(1)
        sils = self.rec_model.inference_(ipts).unsqueeze(1)
...
class DeepGaitV2_NO_Base(nn.Module):

    def __init__(self, model_cfg):
        super(DeepGaitV2_NO_Base, self).__init__()
        mode = model_cfg['Backbone']['mode']
        assert mode in blocks_map.keys()
        block = blocks_map[mode]
