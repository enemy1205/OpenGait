...
class SkeletonGaitPP(BaseModel):
    ...
    def build_network(self, model_cfg):
        # B, C = [1, 4, 4, 1], 2
        in_C, B, C = (
            model_cfg["Backbone"]["in_channels"],
            model_cfg["Backbone"]["blocks"],
            model_cfg["Backbone"]["C"],
        )
        self.inference_use_emb = (
            model_cfg["use_emb2"] if "use_emb2" in model_cfg else False
        )

        self.inplanes = 32 * C
        self.sil_layer0 = SetBlockWrapper(
            nn.Sequential(
        ...
        self.map_layer0 = SetBlockWrapper(
            nn.Sequential(
        ...
        self.sil_layer1 = SetBlockWrapper(
            self.make_layer(
                BasicBlock2D, 32 * C, stride=[1, 1], blocks_num=B[0], mode="2d"
            )
        )
        ...
        self.layer2 = self.make_layer(
            BasicBlockP3D, 64 * C, stride=[2, 2], blocks_num=B[1], mode="p3d"
        )
        self.layer3 = self.make_layer(
            BasicBlockP3D, 128 * C, stride=[2, 2], blocks_num=B[2], mode="p3d"
        )
        self.layer4 = self.make_layer(
            BasicBlockP3D, 256 * C, stride=[1, 1], blocks_num=B[3], mode="p3d"
        )
    ...
    def forward(self, inputs):
        ...
        retval = {
            ...
            "visual_summary": {
                "image/sils": rearrange(pose * 255.0, "n c s h w -> (n s) c h w"),
            },
