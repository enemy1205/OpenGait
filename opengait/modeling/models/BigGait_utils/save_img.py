...
def pca_image(data, mask, root, model_name, dataset, n_components=3, is_return=False):
    ...
    norm_features = (
        ...
        .reshape(ns * hw, 1)
        .repeat(n_components, axis=1)
    )
    norm_features[mask != 0] = pca_features

    if is_return:
        norm_features = norm_features.reshape(1, ns, 64, 32, n_components)[
            ..., :3
        ].transpose(
            0, 1, 4, 2, 3
        )  #
    ...
    s = 20
    assert ns % s == 0
    norm_features = norm_features.reshape(ns // s, s, 64, 32, n_components)[
        ..., :3
    ].transpose(0, 1, 4, 2, 3)
...
def save_image(data, root, model_name, dataset, need="image", mask=None):
    ...
    if "image" in need:
        ...
        for i, id in enumerate(label[:]):
            tmp = os.path.join(
                root_path, str(id).zfill(5), str(seq_type[i]), str(view[i])
            )
...
def save_func(tmp, data, ipts_type="image", mask=None):
    if "image" in ipts_type:
        for i, con in enumerate(data):
            if con.shape[0] == 1:
                if "jet" in ipts_type:
                    im = (cv2.applyColorMap(con[0], cv2.COLORMAP_JET) * 0.5)[
                        ..., ::-1
                    ] + 1.0 * mask[i]
                    # im = mask[i]
...
def save_gif(image_folder, save_folder, name="movie"):
    ...
    imageio.mimsave(
        os.path.join(save_folder, f"{name}.gif"), images, duration=50, loop=0
    )
