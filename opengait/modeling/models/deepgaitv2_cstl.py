...
from ..backbones.cstl_module import MSTE_3D,MSTE,ATA
...
class DeepGait_CSTL(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.mste = MSTE(channels[3],channels[3],16)
        ...
        self.adaptive_aggregation = ATA(channels[3], 16, 16)
    ...
    def forward(self, inputs):
        ...
        out1 = self.layer1(out0) # layer0  layer1都是2d卷积，均为对时间维度操作，并且未降低分辨率
        out2 = self.layer2(out1)
        out3 = self.layer3(out2) # layer2  layer3 各降低了一次分辨率
        out4 = self.layer4(out3) # [n, c, s, h, w]
        
        x = out4.max(-1)[0] + out4.mean(-1)
        
        x = x.permute(0, 2, 1, 3)
        
        t_f, t_s, t_l = self.mste(x) # [n, c, s, p]
        
        aggregated_feature = self.adaptive_aggregation(t_f, t_s, t_l)
        # Temporal Pooling, TP
        outs = self.TP(out_, seqL, options={"dim": 2})[0]  # [n, c, h, w]

        # Horizontal Pooling Matching, HPM
        # feat = self.HPP(outs)  # [n, c, p]
        aggregated_feature = aggregated_feature.permute(1,2,0)
        embed_1 = self.FCs(aggregated_feature)  # [n, c, p]
