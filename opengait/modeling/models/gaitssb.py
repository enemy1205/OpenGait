...
class GaitSSB_Pretrain(BaseModel):
    ...
    def inputs_pretreament(self, inputs):
        if self.training:
            ...
            img_q = [
                np2var(
                    np.asarray([trf(fra) for fra in seq[:batch_size]]),
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            retval = {
                "training_feat": {
                    "softmax1": {"logits": logits1, "labels": labels1},
    ...
    def D(self, p, z):  # negative cosine similarity
        ...
        z = ddp_all_gather(
            z, dim=0, requires_grad=False
        )  # [m, c, p],  m = n * the number of GPUs
...
class GaitSSB_Finetune(BaseModel):
    ...
    def get_optimizer(self, optimizer_cfg):
        ...
        for i, ft_lr in enumerate(self.backbone_lr):
            if ft_lr != 0:
                ft_param_list.append(
                    {
                        "params": getattr(
                            self.Backbone.forward_block, "layer%d" % (i + 1)
                        ).parameters(),
                        "lr": ft_lr,
                    }
                )
        ...
        ft_param_list.append({"params": self.head0.parameters(), "lr": valid_arg["lr"]})
