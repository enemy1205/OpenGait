...
class AxialAttention(nn.Module):
    def __init__(self, channels,dropout=0.1, num_heads=4):
        super().__init__()
        self.temporal_attn = nn.MultiheadAttention(embed_dim=channels, num_heads=num_heads)
        self.spatial_attn = nn.MultiheadAttention(embed_dim=channels, num_heads=num_heads)
    ...
    def forward(self, x):
        ...
        temporal_output, _ = self.temporal_attn(x_temporal, x_temporal, x_temporal)  # [t, b*h*w, c]
        temporal_output = temporal_output.view(t, b, h, w, c).permute(1, 4, 0, 2, 3)  # [b, c, t, h, w]
        # 空间注意力
        x_spatial = x.permute(3, 0, 2, 4, 1).reshape(h, b * t * w, c)  # [h, b*t*w, c]
        spatial_output, _ = self.spatial_attn(x_spatial, x_spatial, x_spatial)  # [h, b*t*w, c]
        spatial_output = spatial_output.view(h, b, t, w, c).permute(1, 4, 2, 0, 3)  # [b, c, t, h, w]
        # 融合
...
class TemporalPyramidPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        for level in self.levels:                
            # 在时间维度上进行池化
            pool = nn.AdaptiveAvgPool3d(output_size=(level, h, w))
            pooled = pool(x)  # [b, c, level, h, w]
            pooled = torch.max(pooled,dim=2)[0]
            out_features.append(pooled)
        out_features_tensor = torch.stack(out_features, dim=0)
        return torch.mean(out_features_tensor,dim=0)

class DeepGaitV2AttenTP(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        # 
        self.attn_layer_3 = AxialAttention(channels[2])
        self.attn_layer_4 = AxialAttention(channels[3])
        ...
        self.TP = PackSequenceWrapper(TemporalPyramidPooling(levels=[1,2,4]))
    ...
    def forward(self, inputs):
        ...
        outs_1 = self.TP(out4_fusion, seqL)
