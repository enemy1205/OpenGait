...
class ParsingGait(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        if self.only_fine_graph:
            ...
            raise ValueError(
                "You should choose fine/coarse graph, or combine both of them."
            )
    ...
    def ParsPartforFineGraph(self, mask_resize, z):
        ...
        ***Fine Parts:
        ...
        for i in range(len(fine_mask_list)):
            ...
            fine_z_list.append(
                (
                    mask.float() * z * self.gammas_fine[i]
                    + (~mask).float() * z * (1.0 - self.gammas_fine[i])
                ).unsqueeze(1)
            )  # split feature map by mask of each class
        fine_z_feat = torch.cat(
            fine_z_list, dim=1
        )  # [n*s, 11, c, h, w] or [n*s, 5, c, h, w]
    ...
    def ParsPartforCoarseGraph(self, mask_resize, z):
        ...
        ***Coarse Parts:
        ...
        for i in range(len(coarse_mask_list)):
            ...
            coarse_z_list.append(
                (
                    mask.float() * z * self.gammas_coarse[i]
                    + (~mask).float() * z * (1.0 - self.gammas_coarse[i])
                ).unsqueeze(1)
            )  # split feature map by mask of each class
        coarse_z_feat = torch.cat(
            coarse_z_list, dim=1
        )  # [n*s, 11, c, h, w] or [n*s, 5, c, h, w]
    ...
    def forward(self, inputs):
        ...
        if self.only_fine_graph:
            outs_ps = self.get_gcn_feat(
                outs_n, outs_ps_fine, self.fine_adj_npy, is_cuda, seqL
            )  # [n, 11, c]
        elif self.only_coarse_graph:
            outs_ps = self.get_gcn_feat(
                outs_n, outs_ps_coarse, self.coarse_adj_npy, is_cuda, seqL
            )  # [n, 5, c]
        elif self.combine_fine_coarse_graph:
            outs_fine = self.get_gcn_feat(
                outs_n, outs_ps_fine, self.fine_adj_npy, is_cuda, seqL
            )  # [n, 11, c]
            outs_coarse = self.get_gcn_feat(
                outs_n, outs_ps_coarse, self.coarse_adj_npy, is_cuda, seqL
            )  # [n, 5, c]
        ...
        retval = {
            ...
            "visual_summary": {"image/pars": pars.view(n * s, 1, h, w)},
