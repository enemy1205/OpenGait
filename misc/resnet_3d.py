...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.layer4 = self._make_layer(
            block, block_inplanes[3], layers[3], shortcut_type
        )
...
def make_3d_layer(in_planes, planes, stride, blocks):
    ...
    if stride != 1 or in_planes != planes:
        downsample = nn.Sequential(
            conv1x1x1(in_planes, planes, stride), nn.BatchNorm3d(planes)
        )
    layers = nn.Sequential()
    layers.append(
        Basic3DBlock(
            in_planes=in_planes, planes=planes, stride=stride, downsample=downsample
        )
    )
    for i in range(1, blocks):
...
class ResNet_1_Stage(nn.Module):
    ...
    def forward(self, x):
        ...
        res = torch.cat(
            [
                F.interpolate(x[i], size=(30, 20), mode="bilinear").unsqueeze(0)
