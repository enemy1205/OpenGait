...
def make_layer(in_planes, planes, stride, blocks):
    ...
    if stride != 1 or in_planes != planes:
        downsample = nn.Sequential(
            conv1x1(in_planes, planes, stride), nn.BatchNorm2d(planes)
        )
    layers = nn.Sequential()
    layers.append(
        BasicBlock(
            inplanes=in_planes, planes=planes, stride=stride, downsample=downsample
        )
    )
    for i in range(1, blocks):
...
class SwinGait3D(nn.Module):
    def __init__(self) -> None:
        ...
        self.conv_block = nn.Sequential(
            BasicConv2d(1, self.in_planes, 3, 1, 1), nn.LeakyReLU(inplace=True)
        )
        self.backbone = nn.Sequential(
