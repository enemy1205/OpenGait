...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_patches, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, linear=False,
                 focusing_factor=3, kernel_size=5):
        super().__init__()
...
class PyramidVisionTransformerV2(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], la_sr_ratios='8421', num_stages=4, linear=False,
                 focusing_factor=3, kernel_size=5):
        super().__init__()
        self.depths = depths
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                ...
                sr_ratio= int(la_sr_ratios[i]), linear=linear,
                focusing_factor=focusing_factor, kernel_size=kernel_size)
