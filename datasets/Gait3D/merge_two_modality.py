        ...
        for _type in os.listdir(id_path):
            ...
            for _view in os.listdir(type_path):
                ...
                for _seq in os.listdir(view_path):
                    ...
                    smpls_seq_path = os.path.join(smpls_path, _id, _type, _view, _seq)
                    output_seq_path = os.path.join(output_path, _id, _type, _view)
                    os.makedirs(output_seq_path, exist_ok=True)
                    link_method(
                        sils_seq_path, os.path.join(output_seq_path, "sils-" + _seq)
                    )
                    link_method(
                        smpls_seq_path, os.path.join(output_seq_path, "smpls-" + _seq)
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sils_path", default="", type=str, help="Root path of raw silhs dataset."
    )
    parser.add_argument(
        "--smpls_path", default="", type=str, help="Root path of raw smpls dataset."
    )
    parser.add_argument(
        "-o",
        "--output_path",
    ...
    parser.add_argument(
        ...
        default="hard",
        type=str,
        choices=["hard", "soft", "copy"],
        help="Link type of output data.",
    )
    args = parser.parse_args()

    merge(
        sils_path=Path(args.sils_path),
        smpls_path=Path(args.smpls_path),
