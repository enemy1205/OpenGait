        ...
        for sub_seq in sid.iterdir():
            ...
            for subfile in os.listdir(sub_seq):
                ...
                if subfile not in os.listdir(dst) and subfile.endswith("_2d_pose.txt"):
                    ...
                    os.symlink(
                        os.path.join(src, subfile), os.path.join(dst, pose_subfile)
                    )
        progress.update(1)
...
def rearrange_test(test_path: Path, output_path: Path) -> None:
    ...
    for sub_seq in probe.iterdir():
        ...
        for subfile in os.listdir(sub_seq):
            ...
            if subfile not in os.listdir(dst) and subfile.endswith("_2d_pose.txt"):
                pose_subfile = "pose_" + subfile
                os.symlink(os.path.join(src, subfile), os.path.join(dst, pose_subfile))
            progress.update(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input_path", required=True, type=str, help="Root path of raw dataset."
    )
    parser.add_argument(
        "-o",
        "--output_path",
        default="GREW_rearranged",
        type=str,
