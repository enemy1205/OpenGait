...
def compare_pcd_rgb_timestamp(pcd_file, rgb_file):
    ...
    rgb_time = float(
        rgb_file.split("/")[-1].replace(".jpg", "")[:10]
        + "."
        + rgb_file.split("/")[-1].replace(".jpg", "")[10:]
    )
...
def imgs2pickle(
    ...
    for index, modality_files in enumerate(img_paths):
        ...
        if modality == "PCDs":
            data = [
                np.asarray(o3d.io.read_point_cloud(points).points)
                for points in data_files
            ]
            pcd_list = data_files
            ...
            # transpose to (C, H W)
            data = [
                cv2.resize(img, (img_size, img_size), interpolation=cv2.INTER_CUBIC)
                for img in imgs
            ]
            ...
            data = [
                cv2.resize(sil, (img_size, img_size), interpolation=cv2.INTER_CUBIC)
                for sil in sils
            ]
            data = np.asarray(data)
        ...
        if modality == "RGB_raw":
            pkl_path = os.path.join(
                dst_path, f"{cnt:02d}-{sinfo[2]}-Camera-Ratios-HW.pkl"
            )
        ...
        if "PCDs" in modality:
            pkl_path = os.path.join(
                dst_path, f"{cnt:02d}-{sinfo[2]}-LiDAR-{modality}.pkl"
            )
            pickle.dump(data, open(pkl_path, "wb"))
        else:
            pkl_path = os.path.join(
                dst_path, f"{cnt:02d}-{sinfo[2]}-Camera-{modality}.pkl"
            )
            pickle.dump(data, open(pkl_path, "wb"))
        cnt += 1
    ...
    for pcd_index in range(len(pcd_list)):
        ...
        for rgb_index in range(len(rgb_list)):
            pcd_t, rgb_t = compare_pcd_rgb_timestamp(
                pcd_list[pcd_index], rgb_list[rgb_index]
            )
    ...
    for index, modality_files in enumerate(img_paths):
        ...
        data_files = modality_files[1]
        data_files = (
            [data_files[index] for index in pcd_indexs]
            if "PCDs" in modality
            else [data_files[index] for index in rgb_indexs]
        )
        ...
        if modality == "RGB_raw":
            pkl_path = os.path.join(
                dst_path, f"{cnt:02d}-sync-{sinfo[2]}-Camera-Ratios-HW.pkl"
            )
            pickle.dump(HWs, open(pkl_path, "wb"))
            cnt += 1
        ...
        if "PCDs" in modality:
            pkl_path = os.path.join(
                dst_path, f"{cnt:02d}-sync-{sinfo[2]}-LiDAR-{modality}.pkl"
            )
            pickle.dump(data, open(pkl_path, "wb"))
        else:
            pkl_path = os.path.join(
                dst_path, f"{cnt:02d}-sync-{sinfo[2]}-Camera-{modality}.pkl"
            )
            pickle.dump(data, open(pkl_path, "wb"))
        cnt += 1
        ...
        for type_ in os.listdir(os.path.join(input_path, id_)):
            for view_ in os.listdir(os.path.join(input_path, id_, type_)):
                for modality in sorted(
                    os.listdir(os.path.join(input_path, id_, type_, view_))
                ):
                    modality_path = os.path.join(
                        input_path, id_, type_, view_, modality
                    )
                    file_names = sorted(os.listdir(modality_path))
                    file_names = [
                        os.path.join(modality_path, file_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=4,
        type=int,
        help="Number of thread workers. Default: 4",
    )
    parser.add_argument(
        "-r", "--img_size", default=64, type=int, help="Image resizing size. Default 64"
    )
    parser.add_argument(
        "-d", "--dataset", default="CASIAB", type=str, help="Dataset for pretreatment."
    )
    parser.add_argument(
        "-v",
        "--verbose",
