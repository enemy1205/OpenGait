...
def lidar_to_2d_front_view(
    points, v_res, h_res, v_fov, val="depth", cmap="jet", saveto=None, y_fudge=0.0
):
        ...
        for seq in os.listdir(os.path.join(input_path, sid)):
            for view in os.listdir(os.path.join(input_path, sid, seq)):
                for img_path in os.listdir(
                    os.path.join(input_path, sid, seq, view, "PCDs")
                ):
                    img_groups[(sid, seq, view, "PCDs_offline_depths")].append(
                        os.path.join(input_path, sid, seq, view, "PCDs", img_path)
                    )
                    total_files += 1
