...
def imgs2pickle(
    ...
    for img_file in sorted(img_paths):
        if verbose:
            logging.debug(
                f"Reading sid {sinfo[0]}, seq {sinfo[1]}, view {sinfo[2]} from {img_file}"
            )
...
def txts2pickle(
    txt_groups: Tuple, output_path: Path, verbose: bool = False, dataset="CASIAB"
) -> None:
    """
    Reads a group of images and saves the data in pickle format.
    ...
    if dataset == "OUMVLP":
        for txt_file in sorted(txt_paths):
            try:
                ...
                if len(jsondata["people"]) == 0:
                    continue
                data = np.array(jsondata["people"][0]["pose_keypoints_2d"]).reshape(
                    -1, 3
                )
        ...
        for txt_file in sorted(txt_paths):
            if verbose:
                logging.debug(
                    f"Reading sid {sinfo[0]}, seq {sinfo[1]}, view {sinfo[2]} from {txt_file}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--pose", default=False, action="store_true", help="Processing pose."
    )
