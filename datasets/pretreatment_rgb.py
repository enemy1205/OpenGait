    ...
    for ty in sorted(os.listdir(os.path.join(src, id))):
        for vi in sorted(os.listdir(os.path.join(src, id, ty))):
            ...
            for img_file in sorted(os.listdir(os.path.join(src, id, ty, vi))):
                ...
                ratios.append(ratio)
                aligned_img = np.transpose(
                    cv2.cvtColor(
                        resize_with_padding(img, (256, 128)), cv2.COLOR_BGR2RGB
                    ),
                    (2, 0, 1),
                )
            ...
            if len(aligned_imgs) > 0:
                ...
                pickle.dump(
                    np.asarray(aligned_imgs),
                    open(os.path.join(output_path, vi + "-aligned-rgbs.pkl"), "wb"),
                )
                pickle.dump(
                    np.asarray(ratios),
                    open(os.path.join(output_path, vi + "-ratios.pkl"), "wb"),
                )
