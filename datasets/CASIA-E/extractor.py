...
def make_pkl_for_one_person(id_, output_path, img_size=64):
    if id_.split(".")[-1] != "tar" or not os.path.exists(
        os.path.join(output_path, id_)
    ):
        return
                ...
                for view in sorted(os.listdir(type_path)):
                    ...
                    for num in sorted(os.listdir(view_path)):
                        ...
                        for file_ in sorted(os.listdir(num_path)):
                            img = cv2.imread(
                                os.path.join(num_path, file_), cv2.IMREAD_GRAYSCALE
                            )
                        ...
                        if len(imgs) > 5:
                            ...
                            pickle.dump(
                                np.asarray(imgs),
                                open(os.path.join(pkl_path, f"{view}.pkl"), "wb"),
                            )
                        else:
...
def extractall(base_path: Path, output_path: Path, workers=1, img_size=64) -> None:
    ...
    with mp.Pool(workers) as pool:
        for _ in pool.imap_unordered(
            partial(
                make_pkl_for_one_person, output_path=output_path, img_size=img_size
            ),
            ids,
        ):
            progress.update(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Base path to CASIA-E zip files",
    )
    parser.add_argument(
        "-o",
        "--output_path",
        type=str,
        required=True,
        help="Output path for extracted files. The pickle files are generated in ${output_path}/opengait/",
    )
    parser.add_argument(
        "-s", "--img_size", default=64, type=int, help="Image resizing size. Default 64"
    )
    parser.add_argument(
    ...
    extractall(
        Path(args.input_path), Path(args.output_path), args.num_workers, args.img_size
    )
