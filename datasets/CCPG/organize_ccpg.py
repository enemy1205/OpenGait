...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sil_path", default="", type=str, help="Root path of raw silhouette dataset."
    )
    parser.add_argument(
        "--rgb_path", default="", type=str, help="Root path of raw RGB dataset."
    )
    parser.add_argument(
        "-o",
        "--output_path",
        ...
        for _type in sorted(os.listdir(os.path.join(args.rgb_path, _id))):
            for _view in sorted(os.listdir(os.path.join(args.rgb_path, _id, _type))):
                ...
                for img_file in sorted(
                    os.listdir(os.path.join(args.rgb_path, _id, _type, _view))
                ):
                    seg_file = img_file.split(".")[0] + ".png"
                    img_path = os.path.join(args.rgb_path, _id, _type, _view, img_file)
                    seg_path = os.path.join(args.rgb_path, _id, _type, _view, seg_file)
                    ...
                    img = np.transpose(
                        cv2.cvtColor(cv2.resize(img, RGB_SIZE), cv2.COLOR_BGR2RGB),
                        (2, 0, 1),
                    )
                    imgs.append(img)
                    segs.append(cv2.resize(seg, RGB_SIZE))
                    aligned_segs.append(aligned_seg)
                    ratios.append(ratio)
                if len(imgs) > 0:
                    output_path = os.path.join(args.output_path, _id, _type, _view)
                    os.makedirs(output_path, exist_ok=True)
                    pickle.dump(
                        np.asarray(imgs),
                        open(os.path.join(output_path, _view + "-rgbs.pkl"), "wb"),
                    )
                    pickle.dump(
                        np.asarray(segs),
                        open(os.path.join(output_path, _view + "-sils.pkl"), "wb"),
                    )
                    pickle.dump(
                        np.asarray(ratios),
                        open(os.path.join(output_path, _view + "-ratios.pkl"), "wb"),
                    )
                    pickle.dump(
                        np.asarray(aligned_segs),
                        open(
                            os.path.join(output_path, _view + "-aligned-sils.pkl"), "wb"
                        ),
                    )
                else:
                    print(
