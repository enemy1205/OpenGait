...
def extractall(base_path: Path, output_path: Path, passwords) -> None:
    ...
    for file_path in tqdm(list(base_path.rglob("Silhouette_*.7z"))):
        ...
        with py7zr.SevenZipFile(file_path, password=passwords) as archive:
            total_items = len([f for f in archive.getnames() if f.endswith(".png")])
            archive.extractall(output_path)

        extracted_files = len(list(output_path.joinpath(file_path.stem).rglob("*.png")))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Base path to OUMVLP .7z files",
    )
    parser.add_argument(
        "-o",
        "--output_path",
    ...
    parser.add_argument(
        "-p", "--password", type=str, required=True, help="password for extracted files"
    )
