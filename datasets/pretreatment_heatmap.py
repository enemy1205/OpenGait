...
class GeneratePoseTarget:
    ...
    def generate_a_limb_heatmap(
        self, arr, starts, ends, start_values, end_values, point_center
    ):
        ...
        for start, end, start_value, end_value in zip(
            ...
            if d2_ab < 1:
                self.generate_a_heatmap(
                    arr, start[None], start_value[None], point_center
                )
                continue
    ...
    def generate_heatmap(self, arr, kps, max_values):
        ...
        if self.with_kp:
            ...
            for i in range(num_kp):
                self.generate_a_heatmap(
                    arr[i], kps[:, i], max_values[:, i], point_center
                )
        ...
        if self.with_limb:
            for i, limb in enumerate(self.skeletons):
                ...
                self.generate_a_limb_heatmap(
                    arr[i], starts, ends, start_values, end_values, point_center
                )
    ...
    def gen_an_aug(self, pose_data):
        ...
        for i in range(num_frame):
            ...
            kpscores = (
                all_kpscores[:, i]
                if self.use_score
                else np.ones_like(all_kpscores[:, i])
            )
...
class HeatmapToImage:
    ...
    def __call__(self, heatmaps):
        ...
        return np.ascontiguousarray(
            np.mean(np.array(heatmaps), axis=-1, keepdims=True).transpose(0, 3, 1, 2)
        )
...
class CenterAndScaleNormalizer:
    ...
    def __init__(
        self, pose_format="coco", use_conf=True, heatmap_image_height=128
    ) -> None:
        """
        Parameters:
    ...
    def __call__(self, data):
        ...
        if self.pose_format.lower() == "coco":
            hip = (pose_seq[:, 11] + pose_seq[:, 12]) / 2.0  # [t, 2]
        elif self.pose_format.split("-")[0].lower() == "openpose":
            hip = (pose_seq[:, 9] + pose_seq[:, 12]) / 2.0  # [t, 2]
        else:
            raise ValueError(
                f"Error value for pose_format: {self.pose_format} in CenterAndScale Class."
            )
        ...
        pose_seq *= (self.heatmap_image_height // 1.5) / (y_max - y_min)[
    ...
    def __init__(self, pad_method="knn", use_conf=True) -> None:
        ...
        if pad_method.lower() == "knn":
            ...
            self.imputer = SimpleImputer(
                missing_values=0.0, strategy="mean", add_indicator=True
            )
        else:
...
class HeatmapAlignment:
    def __init__(
        self, align=True, final_img_size=64, offset=0, heatmap_image_size=128
    ) -> None:
        self.align = align
    ...
    def center_crop(self, heatmap):
        ...
        if self.align:
            ...
            raw_heatmap = raw_heatmap[
                y_top - self.offset : y_btm + 1 + self.offset,
                (self.heatmap_image_size // 2)
                - (height // 2) : (self.heatmap_image_size // 2)
                + (height // 2)
                + 1,
            ]
        raw_heatmap = cv2.resize(
            raw_heatmap,
            (self.final_img_size, self.final_img_size),
    ...
    def __call__(self, heatmap_imgs):
        ...
        heatmap_imgs = np.array(
            [self.center_crop(heatmap_img) for heatmap_img in heatmap_imgs]
        )
...
def GenerateHeatmapTransform(
    ...
    transform = T.Compose(
        [
            GatherTransform(
                base_transform, transform_bone, transform_joint
            )  # [T, 2, H, W]
        ]
    )
...
class TransferDataset(Dataset):
    def __init__(self, args, generate_heatemap_cfgs) -> None:
        ...
        assert self.dataset_name.lower() in [
            "sustech1k",
            "grew",
            "ccpg",
            "oumvlp",
        ...
        self.save_root = os.path.join(
            args.save_root, f"{self.dataset_name}_sigma_{sigma}_{args.ext_name}"
        )
        ...
        if self.dataset_name.lower() == "sustech1k":
            self.all_ps_data_paths = sorted(
                glob(os.path.join(pose_root, "*/*/*/03*.pkl"))
            )
        else:
            self.all_ps_data_paths = sorted(
                glob(os.path.join(pose_root, "*/*/*/*.pkl"))
            )
    ...
    def __getitem__(self, index):
        ...
        if index < 10:
            ...
            for save_heatemapimg_index in range(heatmap_img.shape[0]):
                cv2.imwrite(
                    os.path.join(save_path_img, f"bone_{save_heatemapimg_index}.jpg"),
                    heatmap_img[save_heatemapimg_index, 0],
                )
                cv2.imwrite(
                    os.path.join(save_path_img, f"pose_{save_heatemapimg_index}.jpg"),
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Utility for generating heatmaps from pose data."
    )
    parser.add_argument(
        "--pose_data_path",
        type=str,
        required=True,
        help="Path to the root directory containing pose data (.pkl files, ID-level) files.",
    )
    parser.add_argument(
        "--save_root",
        type=str,
        required=True,
        help="Root directory where generated heatmap .pkl files will be saved (ID-level).",
    )
    parser.add_argument(
        "--ext_name",
        type=str,
        default="",
        help="Extension name to be appended to the 'save_root' for identification.",
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
        required=True,
        help="Name of the dataset being preprocessed.",
    )
    parser.add_argument(
        "--heatemap_cfg_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Local rank for distributed processing, defaults to 0 for non-distributed setups.",
    )
...
if __name__ == "__main__":
    ...
    with open(args.heatemap_cfg_path, "r") as stream:
        ...
        generate_heatemap_cfgs = replace_variables(
            generate_heatemap_cfgs, generate_heatemap_cfgs
        )
    # Create the dataset
    dataset = TransferDataset(args, generate_heatemap_cfgs)

    # Create the dataloader
    dist_sampler = SequentialDistributedSampler(
        dataset, batch_size=1, rank=local_rank, num_replicas=world_size
    )
