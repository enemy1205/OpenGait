...
def get_args():
    parser = argparse.ArgumentParser(
        description="Symlink silouette data and pose data into the same folder for SkeletonGait++ training."
    )
    parser.add_argument(
        "--heatmap_data_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="path of silouette data, must be the absolute path.",
    )
    parser.add_argument(
        "--dataset_pkl_ext_name",
        type=str,
        default=".pkl",
        help="The extent name for .pkl files of silouettes data.",
    )
    parser.add_argument(
        "--output_path", type=str, required=True, help="path of output data"
    )
...
def main():
    ...
    all_silouette_files = sorted(
        glob(os.path.join(silhouette_data_path, f"*/*/*/*{opt.dataset_pkl_ext_name}"))
    )
    ...
    if len(all_heatmap_files) >= len(all_silouette_files):
        for heatmap_file in tqdm(all_heatmap_files):
            ...
            if not os.path.exists(sil_folder):
                ...
                silouette_file = sorted(
                    glob(os.path.join(sil_folder, f"*{opt.dataset_pkl_ext_name}"))
                )[0]
