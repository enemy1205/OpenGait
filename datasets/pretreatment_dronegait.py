...
def imgs2pickle(
    img_groups: Tuple, output_path: Path, img_size: int = 64, verbose: bool = False
) -> None:
    ...
    for img_file in sorted(img_paths):
        if verbose:
            logging.debug(
                f"Reading height {sinfo[0]}, sid {sinfo[1]}, seq {sinfo[2]} , clip {sinfo[3]} from {img_file}"
            )
        ...
        img = cv2.resize(
            img, (int(img_size * ratio), img_size), interpolation=cv2.INTER_CUBIC
        )
    ...
    if to_pickle:
        ...
        dst_path = os.path.join(output_path, *sinfo[:-1])
        ...
        pkl_path = os.path.join(dst_path, f"{sinfo[3]}.pkl")
        if verbose:
...
def pretreat(
    ...
    for img_path in input_path.rglob("*.png"):
        ...
        *_, height, sid, seq, clip, _ = img_path.as_posix().split("/")
        img_groups[(sid, seq, height, clip)].append(img_path)
        total_files += 1
    ...
    with mp.Pool(workers) as pool:
        ...
        for _ in pool.imap_unordered(
            partial(
                imgs2pickle, output_path=output_path, img_size=img_size, verbose=verbose
            ),
            img_groups.items(),
        ):
            progress.update(1)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        level=logging.INFO,
        filename="./pretreatment.log",
        filemode="w",
        format="[%(asctime)s - %(levelname)s]: %(message)s",
    )
    in_path = Path("/home/data/DroneGait/Silhouettes")
    out_path = Path("/home/data/DroneGait/Silhouettes_pkl_fold")
    n_workers = 64
    pretreat(
        input_path=in_path,
